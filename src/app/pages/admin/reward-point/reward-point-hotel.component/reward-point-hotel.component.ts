...
@Component({
    selector: 'reward-point-hotel',
    templateUrl: './reward-point-hotel.component.html',
})
export class RewardPointHotelComponent extends AppList {
    ...
    async getListPoint() {
        ...
        try {
            const dataFormServe: any = await this._rewardRepo.getListPoint((this.page - 1) * this.pageSize, this.limit, 'hotel', body);
    ...
    confirmPoint(point: RewardPoint) {
        this._noti.pushQuestion('Thông tin xác thực', `Tôi muốn cấp điểm cho <strong>${point.customerName}</strong>`, true, 'OK', 'Hủy').then(async (result: any) => {
            if (result.value) {
                this._spinner.show();
                try {
                    await this._rewardRepo.confirmPoint('hotel', point.code);
