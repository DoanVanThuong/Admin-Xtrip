...
@Component({
    selector: 'reward-point-flight',
    templateUrl: './reward-point-flight.component.html',
})
export class RewardPointFlightComponent extends AppList {

    rewardPoints: RewardPoint[] = new Array<RewardPoint>();
    constructor(private _rewardRepo: RewardRepo,
    ...
    async getListPoint() {
        ...
        try {
            const dataFormServe: any = await this._rewardRepo.getListPoint((this.page - 1) * this.pageSize, this.limit, 'flight', body);
    ...
    confirmPoint(point: RewardPoint) {
        this._noti.pushQuestion('Thông tin xác thực', `Tôi muốn cấp điểm cho <strong>${point.customerName}</strong>`, true, 'OK', 'Hủy').then(async (result: any) => {
            if (result.value) {
                ...
                try {
                    await this._rewardRepo.confirmPoint('flight', point.code);
